...
import { generateRegistry, generateScreens, generateControlLoops } from '@idtp/engines';
...
describe('thermal seed registry (doc 07 §4 / doc 08 §4) — quy mô §10', () => {
  it('roll-up: ≥ 3.000 tag · ≥ 600 alarm · ≥ 70 màn hình · ≥ 25 loop', () => {
    const screens = generateScreens(reg, thermalSeedSpec.instances);
    const loops = generateControlLoops(reg);
    // eslint-disable-next-line no-console
    console.log(
      `[seed] tags=${reg.tags.length} alarms=${reg.alarms.length} screens=${screens.length} loops=${loops.length} byScan=${JSON.stringify(reg.byScanClass)} byCell=${JSON.stringify(reg.byCell)}`,
    );
    ...
    expect(loops.length).toBeGreaterThanOrEqual(25);
